...
package body Image is
   ...
   function Destinations_Image (Destination: T.Destinations_T) return String is
   ...
   begin
      ...
      for i in 1..10 loop
         if Destination(i).Ep /= Null then
            Get_IP_Port(Destination(i).EP, IP, Port);
            EP_String := IP & " " & Port;
            Ret_String := ASU.To_Unbounded_String(Integer'Image(Destination(i).Retries));
            Destination_String := EP_String & " " & Ret_String;
         else
            Destination_String := ASU.To_Unbounded_String("Null.");
         end if;
